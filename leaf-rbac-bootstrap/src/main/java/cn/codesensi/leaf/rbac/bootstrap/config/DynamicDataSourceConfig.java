 ...
 * 动态数据源配置。
 * <p>
 * 根据 {@link DatabaseProperties} 中当前选中的数据库类型动态创建 HikariCP 数据源，
...
public class DynamicDataSourceConfig {
     ...
     * 流程：
     * <ol>
     *   <li>从 {@link DatabaseProperties} 获取当前数据库类型的连接配置</li>
     ...
     *
     * @param dbProps     自定义多数据源配置属性
     * @param environment Spring 环境，用于读取 {@code spring.datasource.hikari.*} 全局参数
     * @return 配置完成的 HikariCP 数据源
     * @throws IllegalStateException 当 {@code dbProps.getCurrentConfig()} 返回 null 时抛出
     */
    @Bean
    @Primary
    public DataSource dataSource(DatabaseProperties dbProps, Environment environment) {
        DatabaseProperties.DataSourceConfig config = dbProps.getCurrentConfig();
        if (config == null) {
            throw new IllegalStateException("No configuration found for database type: " + dbProps.getType());
        }
        ...
        log.info("DataSource initialized: {}", dbProps.getType());
        return dataSource;
    }
}
